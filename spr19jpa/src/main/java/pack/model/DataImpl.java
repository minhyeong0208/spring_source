...
public class DataImpl implements DataInterface {
	...
	public List<MemDto> selectDataAll() {
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("hello"); // DB 연결 정보 : 인수 값은 persistence.xml의
...
//			tx.commit();
			...
			/*
			 * list = findAll(em, MemDto.class); // 두 번째 인자로 엔티티 클래스 전달한다. for(MemDto m :
			 * list) { System.out.println(m.getNumber() + " " + m.getName() + " " +
			 * m.getAddr()); }
			 */
	...
	public <T> List<T> findAll(EntityManager em, Class<T> cla) {
		...
		return em.createQuery("select e from " + cla.getSimpleName() + " e", cla).getResultList(); // 테이블에 대한 별명을 'e'로
																									// 설정
	}
